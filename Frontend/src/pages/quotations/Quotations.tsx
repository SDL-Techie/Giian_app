...
//   <Input
//     label="Subject"
//     placeholder="e.g. Commercial Quotation"
//     value={subject}
...
import DataTransferBar from '../../components/common/data-transfer/DataTransferBar';
import PdfActionModal from '../../components/common/pdf/PdfActionModal';
import { fetchProtectedFile } from '../../utils/fileDownload';
// Static, org-wide stamp. Replace this path with wherever your company
// stamp asset actually lives. Unlike the e-sign, the stamp is NOT
// per-user — there is no `stampUrl` field on the User model today.
// If you want a per-user stamp too, add `stampUrl?: string` to the
// User model/type exactly the way `esignUrl` exists, and fetch it the
// same way `esignBlobUrl` is fetched below.
import companyStampImage from '../../../public/assets/company-stamp.png';
import './Quotations.css';
...
export const Quotations: React.FC = () => {
  ...
  // Creator's e-sign — fetched as a protected file (same pattern as
  // receipt PDF preview) and swapped in per-quotation, based on
  // whoever created it (`quotation.createdBy`), not the viewer.
  const [esignBlobUrl, setEsignBlobUrl] = useState('');
  ...
  // E-sign & stamp are shown ONLY once the quotation has been admin-approved,
  // and the e-sign is whichever user's esignUrl is on quotation.createdBy —
  // i.e. dynamic per creator, not a fixed image.
  const showEsignStamp =
    !!viewingQuotation && viewingQuotation.approvalStatus === 'Approved';

  const creatorEsignUrl =
    viewingQuotation && typeof viewingQuotation.createdBy === 'object'
      ? (viewingQuotation.createdBy as { esignUrl?: string })?.esignUrl
      : undefined;

  const creatorName =
    viewingQuotation && typeof viewingQuotation.createdBy === 'object'
      ? (viewingQuotation.createdBy as { name?: string })?.name
      : undefined;

  // Fetch the creator's e-sign image as a protected file whenever an
  // approved quotation is opened. Cleared/aborted on close or switch.
  useEffect(() => {
    let cancelled = false;
    let currentUrl = '';

    const load = async () => {
      if (!showEsignStamp || !creatorEsignUrl) {
        setEsignBlobUrl((old) => {
      ...
      try {
        const blob = await fetchProtectedFile(creatorEsignUrl);
        if (cancelled) return;
        ...
        if (!cancelled) toast.error(err.message || "Failed to load creator's e-sign");
      } finally {
        if (!cancelled) setIsLoadingEsign(false);
      }
    };
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [showEsignStamp, creatorEsignUrl]);

  useEffect(() => {
    return () => {
      if (esignBlobUrl) URL.revokeObjectURL(esignBlobUrl);
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
          ...
          <Table
            ...
            columns={[
              ...
              },
              {
                header: 'Approval',
                accessor: (q) => (
                  <Badge variant={q.approvalStatus === 'Approved' ? 'success' : 'warning'}>
                    {q.approvalStatus === 'Approved' ? 'Approved' : 'Pending'}
                  </Badge>
                ),
              },
              {
      ...
      >
        {viewingQuotation && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            {/* ── E-sign & Company Stamp block ──
                Shown ONLY when approvalStatus === 'Approved'.
                The e-sign image is whichever user created this specific
                quotation (quotation.createdBy.esignUrl) — swaps per
                quotation, not a fixed image. The stamp is a fixed
                company-wide asset. */}
            {showEsignStamp && (
              <div className="quotation-esign-stamp-block">
                <div className="quotation-esign-stamp-inner">
                  {isLoadingEsign && <span className="inline-loading">Loading signature…</span>}
                  {!isLoadingEsign && esignBlobUrl && (
                    <div className="quotation-esign-col">
                      <img src={esignBlobUrl} alt={`${creatorName || 'Creator'}'s signature`} className="quotation-esign-img" />
                      <span className="quotation-esign-caption">{creatorName || 'Authorized Signatory'}</span>
                    </div>
                  )}
                  {!isLoadingEsign && !esignBlobUrl && (
                    <span className="quotation-esign-caption">{creatorName || 'Authorized Signatory'}</span>
                  )}
                  <img src={companyStampImage} alt="Company Stamp" className="quotation-stamp-img" />
                </div>
              </div>
            )}

            <div style={{ display:'flex', justifyContent:'flex-end', flexWrap:'wrap', gap:'10px', marginTop:'8px' }}>
              {/* Approve button: unlocks the creator's e-sign + stamp on
                  both this modal and the generated PDF. */}
              {user?.isAdmin && viewingQuotation.approvalStatus !== 'Approved' && (
                <button
                  className="btn btn-secondary btn-md"
                  disabled={isApproving}
