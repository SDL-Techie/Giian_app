...
export const Purchases: React.FC = () => {
  ...
  // Download tracking (per-file loading state, keyed by an id we build ourselves)
  ...
  // A plain <a href={url} download> makes an unauthenticated browser
  // request, so the backend rejects it with "Access Denied. Login
  // Required". Instead we fetch the file through purchaseService (which
  // uses the shared axios instance and therefore attaches the auth
  // token/cookie automatically), pull it down as a blob, and then
  // trigger a save-as from that blob.
  // ---------------------------------------------------------------------
  const handleDownloadDocument = async (fileUrl: string, filename: string, key: string) => {
    if (!fileUrl) return;
    setDownloadingKey(key);
    try {
      const blob = await purchaseService.downloadPurchaseDocument(fileUrl);
                ...
                <div className="table-actions">
                  {p.invoiceFileUrl ? (
                    <button
                      ...
                      className="action-icon-btn btn-view"
                      title="Download Uploaded Invoice Document"
                      disabled={downloadingKey === p._id}
                      onClick={() =>
                        handleDownloadDocument(
                          p.invoiceFileUrl as string,
                          `${p.purchaseNo || 'invoice'}.pdf`,
                          p._id
                        )
                      }
                    >
              ...
              {invoiceFiles.length === 0 && (
                <div className="document-upload-row">
                  ...
                  <button
                    type="button"
                    className="add-document-btn"
                    onClick={() => setInvoiceFiles([null as any])}
                    title="Add another document"
                  >
                    +
                  </button>
          ...
          >
            ...
            <div className="form-grid-2">
              ...
              <div>
                ...
                <div>
                  <Badge variant={selectedPurchase.status === 'Active' ? 'success' : 'danger'}>
                    {selectedPurchase.status}
                  </Badge>
                </div>
              </div>
            ...
            {/* Purchase Items */}
            <div>
              <h3 style={{ marginBottom: '10px', fontSize: 'var(--font-size-md)' }}>
                Purchase Items
              </h3>
            ...
            <div className="purchase-documents-section">
              <div className="purchase-documents-header">
                ...
                <Badge variant="success">
                  {selectedPurchase.invoiceFiles?.length || 0}{' '}
                  {selectedPurchase.invoiceFiles?.length === 1 ? 'Document' : 'Documents'}
                </Badge>
              </div>

              {selectedPurchase.invoiceFiles && selectedPurchase.invoiceFiles.length > 0 ? (
                <div className="purchase-documents-list">
                  {selectedPurchase.invoiceFiles.map((doc, index) => {
                    const key = doc.id ? String(doc.id) : `${selectedPurchase._id}-${index}`;
                    return (
                      <div key={key} className="purchase-document-item">
                        <div className="purchase-document-info">
                          <span className="purchase-document-icon">📄</span>

                          <div style={{ display: 'flex', flexDirection: 'column', minWidth: 0 }}>
                            <span className="purchase-document-name" title={doc.filename}>
                        ...
                        <button
                          ...
                          onClick={() =>
                            handleDownloadDocument(doc.url, doc.filename || `document-${index + 1}`, key)
                          }
                        >
                          <Download size={16} />
                        </button>
